...
def calculate_mu_features(df, k=6):
    ...
    team_history = {team: {"corners": [], "shots_on_target": [], "total_shots": [], "goals": []} for team in teams}
...
def calculate_head_to_head_features(df, window=5):
    ...
    for idx, row in df.iterrows():
        ...
        # *** ATUALIZAR HISTÓRICO APÓS FEATURE CREATION (anti-leakage) ***
        h2h_history[h2h_key].append({"result": row["Result"], "home_goals": row["FTHG"], "away_goals": row["FTAG"]})
...
def calculate_league_position_features(df):
    ...
    for idx, row in df.iterrows():
        ...
        sorted_teams = sorted(standings.items(), key=lambda x: (x[1]["points"], x[1]["gd"], x[1]["gf"]), reverse=True)
...
def calculate_interaction_features(df_features):
    ...
    interactions["strength_balance"] = np.abs(interactions["position_form_home"] - interactions["position_form_away"])
...
def calculate_team_stats(df):
    ...
    df_features = pd.concat([df_features, df_form[["form_diff", "home_form", "away_form"]]], axis=1)
