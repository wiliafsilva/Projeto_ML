...
def train_models(df_train, df_test):
    ...
    try:
        ...
        gan = GANClassifier(epochs=50, batch_size=64, lr=1e-3, noise_dim=32, gen_hidden=(128, 128), disc_hidden=(128, 128), random_state=42)
    ...
    for name, model in models.items():
        ...
        # Transform features
        X_train_baseline_scaled = pd.DataFrame(scaler_baseline.transform(X_train_base), columns=X_train_base.columns, index=X_train_base.index)
        X_test_baseline_scaled = pd.DataFrame(scaler_baseline.transform(X_test), columns=X_test.columns, index=X_test.index)
        ...
        # Training
        clf_base, metrics_base = _train_and_eval(model, X_train_baseline_scaled, y_train_base, X_test_baseline_scaled, y_test, name, use_sample_weight=use_sw)
        ...
        if np.any(np.isnan(X_train_latent)) or np.any(np.isnan(X_test_latent)) or np.any(np.isinf(X_train_latent)) or np.any(np.isinf(X_test_latent)):
        ...
        clf_latent, metrics_latent = _train_and_eval(model, X_train_latent_df, y_train_base, X_test_latent_df, y_test, name, use_sample_weight=use_sw)
    ...
    rf_base = RandomForestClassifier(n_estimators=200, max_depth=10, min_samples_split=2, min_samples_leaf=4, random_state=42, class_weight="balanced")
    xgb_base = XGBClassifier(eval_metric="mlogloss", n_estimators=50, max_depth=3, learning_rate=0.05, subsample=0.7, colsample_bytree=0.7, random_state=42)
    nb_base = GaussianNB()
    ...
    voting_weighted = VotingClassifier(
        estimators=[
            ("rf", RandomForestClassifier(n_estimators=200, max_depth=10, min_samples_split=2, min_samples_leaf=4, random_state=42, class_weight="balanced")),
            ("xgb", XGBClassifier(eval_metric="mlogloss", n_estimators=50, max_depth=3, learning_rate=0.05, subsample=0.7, colsample_bytree=0.7, random_state=42)),
            ("nb", GaussianNB()),
        ],
        voting="soft",
    ...
    for season_name, season_value in seasons_info:
        ...
        for name, info in results.items():
            ...
            seasonal_results[season_name][name] = {"accuracy": acc_season, "precision": prec_season, "recall": rec_season, "f1": f1_season, "rps": rps_season, "n_samples": len(y_season)}
    ...
    results_metadata = {
        ...
        "test_seasons": {"2014-2015": len(df_test[df_test["Season"] == 2015]), "2015-2016": len(df_test[df_test["Season"] == 2016]), "All": len(df_test)},
