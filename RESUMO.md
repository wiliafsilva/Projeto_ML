...
O treinamento foi conduzido exclusivamente sobre dados de 2005-2014 (3.420 partidas) para evitar data leakage. A função de perda utilizada foi Mean Squared Error (MSE) entre entrada e reconstrução, otimizada através do algoritmo Adam com learning rate de 0,001. O modelo foi treinado por 100 epochs com batch size de 32 amostras. Previamente ao treinamento, todas as 43 features foram normalizadas via StandardScaler com média zero e desvio padrão unitário, sendo o scaler ajustado exclusivamente sobre o conjunto de treinamento 2005-2014. Esta normalização foi armazenada para reutilização durante a extração de features latentes sobre dados de teste, garantindo consistência.

Uma representação compacta do fluxo do autoencoder é a seguinte:
$$X_{43} \longrightarrow \text{Densa}(32, \text{ReLU}) \longrightarrow \text{Densa}(16, \text{ReLU}) \implies Z_{16} \text{ (Espaço Latente)}$$
...
Quatro modelos individuais foram selecionados como base para análise comparativa. Random Forest foi treinado com 50 estimadores e profundidade máxima de 5, utilizando pesos balanceados para mitigar desbalanceamento de classes. XGBoost foi configurado com 200 estimadores, profundidade máxima de 3, learning rate de 0,01 e subamostragem de 80% das amostras por iteração, técnicas que promovem regularização e reduzem overfitting. SVM utilizou kernel RBF (Radial Basis Function) com parâmetro de regularização C=0,1 e parâmetro de largura de kernel γ=0,001, ambos com pesos balanceados para lidar com desproporção de classes. Gaussian Naive Bayes foi implementado com parâmetro de suavização de variância var_smoothing=1e-9 (padrão do scikit-learn), evitando divisão por zero em variâncias muito próximas a zero.

Adicionalmente, três estratégias de ensemble foram implementadas para explorar sinergia entre modelos. Voting_Equal combinava as predições de Random Forest, XGBoost e Gaussian Naive Bayes com pesos iguais [1/3, 1/3, 1/3], permitindo votação igualitária entre os três. Voting_Weighted atribuía pesos diferentes [0,4, 0,3, 0,3] para privilegiar Random Forest baseado em desempenho preliminar. Stacking utilizava meta-learner com regressão logística, treinado a partir das predições de probabilidade dos três modelos base, aprendendo qual combinação das predições bases produzia melhor resultado.
...
Com ressalva importante, Random Forest apresenta limitação crítica quanto ao desbalanceamento de classes: tende a subestimar a classe minoritária (empates) e, em alguns experimentos, chega a prever poucos ou nenhum empate no conjunto de teste. Este comportamento reduz recall para a classe D (empate) e inflaciona precision para H e A, fenômeno onde o modelo aprende que é mais seguro predizer vitória quando incerteza prevalece. XGBoost, em contraste, mitigou esse viés de forma mais efetiva através de suas penalidades de regularização. Técnicas como oversampling (e.g., SMOTE) ou ajuste de limiares de decisão (threshold tuning) podem ser empregadas para mitigar esse viés e melhorar recall para empates.
